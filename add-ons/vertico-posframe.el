...
(defvar vertico-posframe-evil-state 'insert)
(add-hook 'evil-insert-state-entry-hook (lambda () (setq vertico-posframe-evil-state 'insert)))
(add-hook 'evil-normal-state-entry-hook (lambda () (setq vertico-posframe-evil-state 'normal)))
(add-hook 'evil-visual-state-entry-hook (lambda () (setq vertico-posframe-evil-state 'visual)))
...
(defun vertico-posframe--show (buffer window-point)
  ...
  (with-selected-window (vertico-posframe-last-window) ;Some posframe poshandlers need infos of last-window.
    ...
    (apply #'posframe-show
           buffer
           :cursor (if (eq vertico-posframe-evil-state 'insert)
		       '(bar . 2)
		     'box)
